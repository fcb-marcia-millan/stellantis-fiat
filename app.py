...
.dash-sub { font-size: 11px; color: #ff1430; letter-spacing: 2px; text-transform: uppercase; margin-top: 4px; }
.dash-badge {
    background: rgba(0,136,204,0.12); border: 1px solid #ff1430;
    color: #ff1430; font-size: 10px; padding: 4px 12px;
...
.kpi-bar-fill { height: 100%; background: #ff1430; border-radius: 1px; }
.section-title {
...
BASE = dict(
    ...
    colorway=["#ff1430","#ff1430","#00aadd","#73bf69","#fade2a","#ff780a"],
)
...
if pagina == "General":
    ...
    with col_anio:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color="#ff1430", marker_line_width=0,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        colors  = ["#ff1430","#ff1430","#00aadd","#73bf69","#fade2a","#ff780a","#e02f44","#555570"]
    ...
    with col2:
        ...
        bar_colors = ["#555570" if m == SIN_DATO else "#ff1430" for m in mod_uni["am_modelocl"]]
        fig2 = go.Figure(go.Bar(
    ...
    if "mes_compra" in df.columns and not df.empty:
        ...
        if not trend.empty:
            fig3 = px.line(
                ...
                color_discrete_sequence=["#ff1430","#ff1430","#00aadd","#73bf69",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "cl_dir_provincia" not in df.columns:
        ...
        with col1:
            ...
            bar_colors = ["#555570" if p == SIN_DATO else "#ff1430" for p in prov_df["cl_dir_provincia"]]
            fig = go.Figure(go.Bar(
        ...
        with col2:
            if "cl_dir_localidad" in df.columns:
                ...
                loc_df = (df.groupby("cl_dir_localidad")["cl_k_cliente"]
                          ...
                          .sort_values("n", ascending=True))
                bar_colors2 = ["#555570" if l == SIN_DATO else "#ff1430" for l in loc_df["cl_dir_localidad"]]
                fig2 = go.Figure(go.Bar(
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "Gender" not in df_gen.columns:
        ...
        for g in generos_reales:
            ...
            col = COLOR_MAP_GEN.get(g, "#ff1430")
            kpi_html += f"""
            <div class="kpi-card">
        ...
        with col1:
            ...
            pie_colors = [COLOR_MAP_GEN.get(g, "#ff1430") for g in gen_df["Gender"]]
            fig = go.Figure(go.Pie(
