...
st.set_page_config(
    page_title="Fiat - Base de Clientes",
    page_icon="https://www.fiat.com.ar/content/dam/fiat/nova_home/logos/logo_header_hub_fiat.svg",
    layout="wide",
...
def load_data():
    ...
    # Empresa: convertir a numérico, 1 = empresa, resto = particular
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:16px 0 8px 0">
      <img src="https://www.fiat.com.ar/content/dam/fiat/nova_home/logos/logo_footer_hub_fiat.svg" width="100"><br>
...
# df_time: copia de df con filtro temporal (solo si checkbox está activo)
df_time = df.copy()
...
<div class="dash-header">
  <div>
    <div class="dash-title">Stellantis: Fiat</div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if "empresa" not in df_raw.columns:
        ...
        # IMPORTANTE: Los KPIs de empresa se calculan SIEMPRE sobre df_raw (sin filtros)
        # Solo los gráficos internos usan df (con filtros)
        
        df_corp = df_raw[df_raw["empresa"] == 1].copy()
        
        # Clientes únicos empresa: DISTINCT DNI de registros con empresa=1
        clientes_unicos_corp = df_corp["cl_k_cliente"].nunique() if "cl_k_cliente" in df_corp.columns else 0
        
        # Registros corporativos: suma de todos los 1 en empresa
        registros_corp = int((df_raw["empresa"] == 1).sum())
        
        # % sobre total: registros_corp / total registros (sin filtro)
        total_registros = len(df_raw)
        pct_corp = round(registros_corp / total_registros * 100, 1) if total_registros > 0 else 0
        ...
        <div class="kpi-grid">
          ...
          <div class="kpi-card">
            ...
            <div class="kpi-sub">Registros empresa / Total registros</div>
        ...
        with col1:
            st.markdown('<p class="section-title">Modelos preferidos (empresas)</p>', unsafe_allow_html=True)
            ...
            fig.update_layout(xaxis=dict(type="category", gridcolor="#1e1e30", linecolor="#1e1e30"),
                              title=dict(text="Modelos preferidos por empresas", font=dict(size=12), x=0))
            st.plotly_chart(fig, use_container_width=True, config=NO_MB)

        with col2:
            if "cl_dir_provincia" in df_corp.columns:
                st.markdown('<p class="section-title">Empresas por provincia</p>', unsafe_allow_html=True)
                ...
                fig2.update_layout(**layout(280, mr=80, mt=36),
                                   title=dict(text="Top 8 provincias (empresas)", font=dict(size=12), x=0))
                st.plotly_chart(fig2, use_container_width=True, config=NO_MB)

        if "mes_compra" in df_corp.columns and not df_corp.empty:
            ...
            trend_corp = df_corp_time.groupby("mes_compra").size().reset_index(name="n").sort_values("mes_compra")
            if not trend_corp.empty:
                fig3 = px.line(trend_corp, x="mes_compra", y="n",
                               color_discrete_sequence=["#fade2a"])
                fig3.update_traces(line_width=2)
                fig3.update_layout(**layout(200, mr=12, mb=50))
                st.plotly_chart(fig3, use_container_width=True, config=NO_MB)
        ...
        resumen_corp = (df_corp.groupby("am_modelocl", dropna=False)
                        .agg(Registros=("am_modelocl","size"),
                             Clientes_unicos=("cl_k_cliente","nunique"))
                        .reset_index()
                        .rename(columns={"am_modelocl":"Modelo","Clientes_unicos":"Clientes únicos"})
